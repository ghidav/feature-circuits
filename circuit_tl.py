...
from attribution_tl import patching_effect, jvp
...
from atp import AtPTransformer
...
def get_circuit_cluster(dataset,
    ...
    attns = [layer.self_attn for layer in nl]
    mlps = [layer.mlp for layer in nl]
    resids = [layer for layer in nl]
    dictionaries = {}
...
if __name__ == '__main__':
    ...
    model = AtPTransformer.from_pretrained(args.model, device=device)
    ...
    if args.dict_id == 'id':
        ...
        for i in range(nl):
            dictionaries[attns[i]] = IdentityDict(nh * d_head)
    ...
    if not args.plot_only:
        ...
        with open(f'{circuit_dir}/{save_basename}_dict{args.dict_id}_node{args.node_threshold}_edge{args.edge_threshold}_n{num_examples}_agg{args.aggregation}.pt', 'wb') as outfile:
            t.save(save_dict, outfile)

    else:
        with open(f'{circuit_dir}/{save_basename}_dict{args.dict_id}_node{args.node_threshold}_edge{args.edge_threshold}_n{num_examples}_agg{args.aggregation}.pt', 'rb') as infile:
    ...
    if args.aggregation == "none":
        example = model.to_str_tokens(examples[0]["clean_prefix"])
        plot_circuit_posaligned(
            nodes, 
            edges,
            layers=nl, 
            length=args.example_length,
            example_str_tokens=example,
            node_threshold=args.node_threshold, 
            edge_threshold=args.edge_threshold, 
            pen_thickness=args.pen_thickness, 
            annotations=annotations, 
            save_dir=f'{plot_dir}/{save_basename}_dict{args.dict_id}_node{args.node_threshold}_edge{args.edge_threshold}_n{num_examples}_agg{args.aggregation}'
        )
    else:
        plot_circuit(
            nodes, 
